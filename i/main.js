...
$(document).ready(function () {
    ...
    setInterval(function () {
        if (isMouseDown) {
            ...
            let fSpeed = $('#jogSpeed').val();
            let fSize = $('#jogSize').val();
            ...
            gcX = Math.round(gcX * fSize * 1000) / 1000;
            gcY = Math.round(gcY * fSize * 1000) / 1000;
    ...
    socket.on('config', function (data) {
        if (data.showWebCam) {
            ...
            let webroot = window.location.protocol + '//' + window.location.hostname;
            //console.log(webroot);
    ...
    socket.on('ports', function (data) {
        ...
        const optional = (v) => v || '?';
        /**
         * @var {{pnpId?: string; vendorId?: string; productId?: string}} port
         */
        for (const port of data) {
            const details = (
                Object.entries(port)
                    .filter(([prop]) => !['idx', 'path'].includes(prop))
                    .map(([prop, value]) => `${prop}: ${optional(value)}`)
                    .join(', ')
            );
            const product = (port.vendorId === '1a86' && port.productId === '7523') ? ` (CH340 chip)` : '';
            const label = `${port.path} @ ${details}${product}`;

            $('#choosePort').append('<option value="' + port.idx + '">' + label + '</option>');
        }
    ...
    window.sendCommand = (command) => socket.emit('gcodeLine', { line: command });
